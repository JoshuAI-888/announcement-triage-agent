...
## C.3 v3 eval — RESOLVED (credits added, re-run)
result: **v3 grounded 0.374** (from v2's 0.164 — the forced-verbatim rule, +128%), precision 0.408, recall 0.939, P@10 0.800, confidently-wrong 0.218. NZ$12.30, 3 runs; 6/660 calls hit transient errors and degraded to sentinels (resilience held, run completed). **Batch C complete — the v1/v2/v3 scorecard shows clean movement, each prompt change moving its target metric.** Honest limitation: even v3 grounds only 37% of quotes — Haiku struggles to copy verbatim despite the explicit rule. README.md and the failure taxonomy NOT written — owner's, as instructed.
...
## Follow-on: tiered PDF handling + Claude OCR + portal (2026-08-04, commit 5a8f984)
Root cause of the "truncation": ARS annual-report PDFs were ingested as raw %PDF byte streams (4 caches at 12-77MB). Replaced the blanket skip with a tiered pipeline in EdgarAdapter._fetch_pdf_document: skip-by-form (ARS, duplicates the 10-K) → pypdf text extraction (event forms <15MB) → Claude native-PDF OCR (scanned/no-text, PINNED to Claude regardless of run.provider, best-effort/never-raises) → metadata placeholder (never drops a filing; carries 8-K item codes; routes to Needs-a-look). New dep: pypdf 6.14.2 (BSD, pure-Python, zero transitive — the only S6 add). OCR toggle pdf.ocr_enabled (default on, portal-editable, persisted, excluded from the eval fingerprint). Decision log out/pdf_log.jsonl committed alongside run_log, read at portal /pdf-log. Portal also: read-only system-prompt view (/api/system-prompt). Purged 185MB stale garbage caches; regenerated the 4 ARS gold rows (source-facts + gold_corpus bodies; labels untouched, S4). Offline suite 17/18 (check_batch pre-existing red). Live-verified: OCR transcribed an image-only PDF accurately at NZ$0.0035; toggle-off makes zero API calls; deploy READY, routes gated, no runtime errors. See decisions.md for full rationale.

## Follow-on: descriptive self-explaining brief (2026-08-05, commit 6f04c33)
Owner wanted each item to explain itself and no internal codes in the reader's view. Built once in the Python renderer (portal iframes the same brief HTML, so email + portal for free): per material item — company name + industry (EDGAR SIC), an AI-generated business/competitive-edge line (Claude-haiku, cached in data/company_profiles.json, caveated, no advice), latest price + ▲/▼ vs prior close + a mail-safe HTML bar sparkline (Twelve Data, material-only, best-effort/never-raises). Single plain-English flag vocabulary src/flags.py mirrored byte-identically in dashboard/lib/flags.ts (G2→"Unverified quote", etc.); a check asserts no raw G#_ literal survives into rendered output. New per-run artifact out/filings/<date>.json (every classified filing: type, classification, why, clickable source) → native sortable portal /filings ("This run") + a compact table in the email. Built concurrently by two Sonnet subagents on disjoint files (Python vs dashboard/) against frozen contracts. Live-verified with the owner's real TWELVEDATA key.

## Follow-on: two production bugs found + fixed (2026-08-05)
1. **Silent brief drop (024070e).** Since 5a8f984 the CI commit step listed out/pdf_log.jsonl in one `git add a b c`; git add is atomic on pathspec errors, so on any day with no PDF filing (the common case) the missing pdf_log made git stage NOTHING — the brief, out/filings artifact and run_log append were all silently dropped ("no changes to commit"). That's why the descriptive brief never reached the portal. Fix: add each path independently in a loop. Verified end-to-end (a forced run committed the 59-filing brief + artifact).
2. **Vercel data-state ERRORs (f9f67d3 → df4491f).** Every run pushes state to the orphan data-state branch (no dashboard/ dir); Vercel auto-built it and failed "Root Directory 'dashboard' does not exist" each run. Production (main) was never affected. First tried git.deploymentEnabled on main's vercel.json — proven ineffective (Vercel reads that from the PUSHED branch). Real fix: the workflow now writes a vercel.json with deploymentEnabled:false ONTO the data-state branch itself. Verified: the next data-state push was skipped, no deployment created.

## Follow-on: Run-now progress UX + live run history (2026-08-05, commit 716144a)
New GET /api/run-status reads daily-brief workflow runs (and the active run's job steps) from the GitHub Actions API. The "Run now" button now tracks the run it dispatches — a progress bar with elapsed time, the current step name (step n/total), a logs link, through to a success/skipped/failed line. New live "Workflow runs" table on the History page lists ongoing AND finished runs (queued / in progress / success / skipped / failed), surfacing runs that never reach run_log.jsonl. Also bumped actions/checkout@v4→v5, setup-python@v5→v6 (Node 20 deprecation). Verified live against the real repo via a gh token.

## Follow-on: delivery routine + "SEC announcement brief" rename (2026-08-05)
Honest capability boundary: the truly device-independent recurring delivery is a claude.ai routine (owner creates it in the web UI — not creatable from Claude Code); the portal's /api/gmail/draft is a Phase-2 stub. So: a local scheduled task `sec-announcement-brief` (06:54 NZT daily) drafts the latest brief to Gmail review-only (never sends), plus the owner got paste-ready claude.ai routine steps. Renamed to "SEC announcement brief" (email heading render_email.py + brief.py; the task). Today's brief drafted into Gmail directly (verified in Drafts). Also restored the substantive 2026-08-05 brief that manual Run-now re-runs had clobbered with an empty version (from 53fb25f) — follow-up chip filed to guard publish() against same-day empty overwrite.

## Follow-on: How-it-works FAQ + plain-language models + SEC branding (2026-08-06, commit 9b8c9bf)
New portal /faq page: intent, end-to-end flow, architecture (two schedulers), digest vs intraday, how confidence is rated, what each AI model actually does (Haiku bulk classifier / Opus escalation, plain English via new lib/models.ts), the guardrail flags (reuses FLAG_VOCAB), and a key-terms glossary. Run history: each completed run links "view brief" to the actual brief HTML it produced (/api/versions/<date>.email.html) and the Model column shows a plain-language role on hover. Renamed the email eyebrow + portal brand to "SEC Announcement Triage". Verified in-browser against the live repo.

## Follow-on: morning digest failing — Actions minutes exhausted (2026-08-06, commit 32fcb89)
Two morning digests failed with "the job was not acquired by Runner of type hosted", and only 12 of 96 scheduled ticks ran in 24h — the signature of an exhausted GitHub Actions minutes/spending limit on a private repo. Root inefficiency: `*/15 * * * *` = 96 runs/day, but the config is a DAILY digest with intraday OFF, so 95/96 were pure "skip" runs that still spun a runner (~40s) and burned minutes. Fix: cron scoped to `0,20,40 17,18 * * *` — covers the 06:00 NZT digest tick in both NZST (18:00 UTC) and NZDT (17:00 UTC) with retry redundancy; 6 runs/day (~16x fewer). This prevents recurrence but does NOT restore this billing cycle — the OWNER must check GitHub → Settings → Billing (Actions minutes / raise spending limit, or wait for the cycle reset), or make the repo public for unlimited Actions minutes. Ties into the cost-optimisation review: the every-15-min cron was the single biggest CI-minutes waste.

## Follow-on: unique run identity + configurable lookback window + backfill + "Daily digest" rename (2026-08-07)
Owner asks: every run uniquely identified (digest reruns were clobbering the bare-date brief/filings), a configurable lookback window (last N days of SEC filings) used by ALL runs and shown on the portal, manual backfill of a past window isolated from the live watermark, portal always shows the latest run, and rename user-facing "digest" → "Daily digest". Locked decisions: window applies to all runs incl. the daily digest; reruns reuse stored classifications (cheap); backfill leaves the live watermark untouched.

Built via two parallel Sonnet subagents (Python vs dashboard/, disjoint files) against a frozen contract, Opus orchestrating (freeze + integrate + verify). Note: the subagents twice refused to edit because a stale plan-mode system-reminder leaked into their context while the main session was still technically in plan mode — correct behavior on their part; once plan mode was genuinely exited, fresh agents built cleanly. Each stream had already written a detailed per-stream plan, so the fresh agents executed with no re-derivation.

Design shipped:
- **run_id** = UTC exec time to the second (`YYYY-MM-DDTHH-MM-SS`); brief/filings/markdown all named `<run_id>.*` for EVERY kind — same-day reruns never clobber. Removed the "digest = bare date" special-case (src/render_email.py, src/brief.py, src/run.py `_filings_filename`). Filings JSON + run_log carry `run_id`, explicit `kind` (digest|intraday|backfill), `window{as_of,lookback_days,since,until}` (+ run_log `reused`, `window_days`, `as_of`).
- **Lookback window** `schedule.lookback_days` (int, default 1; UI 1/2/5/7) across all four schema surfaces (config_schema.py + regenerated runtime_config.schema.json + dashboard schema.ts/types.ts, validator tolerates absence → 1). fetch.py window is `now - lookback_days` (`since`/`until`/`advance_watermark` params); edgar.py adds the upper bound threaded through poll/fetch_ticker/_extract_filings.
- **Reuse-from-cache**: new `classification_cache(announcement_id, prompt_version, result_json…)` table + get/put in store.py; run_pipeline rehydrates a FULLY-verified Classification on a cache hit (skips verify() — not idempotent) and only classifies genuinely-new filings; ALL window records go into the brief (covers the whole window, not just newly-seen). Monotonic guard added to set_watermark.
- **Backfill**: `--as-of YYYY-MM-DD` + `--lookback-days N` CLI; uses the real fetch() with `advance_watermark=False` + `_load_window` so the brief covers cached+new; never moves the live watermark; run_id = actual now (sorts as latest in the portal). daily-brief.yml workflow_dispatch inputs `as_of_date`/`lookback_days` → CLI.
- **Portal**: widened BRIEF_NAME_RE/FILINGS_NAME_RE to accept the seconds segment; briefMeta labels kind via a run_id→kind join off run_log (fallback to old T-heuristic); KIND_LABEL {"digest":"Daily digest","intraday":"Intraday","backfill":"Backfill"} used everywhere a kind is shown; kind unions widened to include "backfill" (.badge.purple); "Backfill…" trigger (date + lookback) on the topbar wired to /api/run-now body → dispatch inputs; Config Schedule section gained the "Lookback window (days)" selector; WorkflowRuns "view brief" link now resolves server-side via run_id join (nullable, omits rather than guessing a dead link).

Verification: offline suite 20/21 (only the known-red check_batch; new/rewritten checks green — run-id uniqueness, since/until window, reuse-cache=0 LLM calls, lookback_days round-trip, backfill heading); portal tsc + next build clean; cross-stream filings/run_log field shapes confirmed identical Python↔TS; Python smoke test produced 3 distinct run_id briefs (no clobber) with a correct backfill window payload; portal renders the Backfill button + config lookback. Supersedes the old same-day-clobber follow-up task. LIVE CI end-to-end still PENDING — GitHub Actions is blocked account-level (runner-not-acquired persists even after the repo was made public; owner must fix GitHub billing/spending limit).
